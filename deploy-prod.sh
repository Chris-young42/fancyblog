...
docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" run --rm backend pnpm exec prisma migrate deploy

echo "[deploy] Seeding database..."
docker compose --env-file "${ENV_FILE}" -f "${COMPOSE_FILE}" run --rm backend pnpm exec prisma db seed
